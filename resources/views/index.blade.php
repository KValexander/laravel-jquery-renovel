...
<head>
	...
	<!-- Подключение плагина для jquery jquery.cookie -->
	<script src="{{ asset('public/spa/library/jquery.cookie.js') }}"></script>
	...
	<!--
		...
		* 	Методы:
			* auth.get_auth() - получение данных авторизации
				* auth.auth.token - переменная токена
				* auth.auth.user_id - переменная идентификатора пользователя
				* auth.auth.role - переменная роли пользователя
			* auth.auth_check(callback) - проверка на то, является ли пользователь авторизованным, или нет
				функция обратного вызова, в случае неавторизованности перенаправление на страницу ошибки авторизации
			* auth.moderation_check(callback) - проверка на то, является ли пользователь администратором/модератором, или нет
				функция обратного вызова, в случае неавторизованности перенаправление на страницу ошибки модерации

		* request - экземпляр класса Request
				для работы с запросами к серверу
		* 	Методы:
			* request.get(callback, data, url) - отправка данных серверу методом GET
			* request.post(callback, data, url) - отправка данных серверу методом POST
				явлются функциями обратного вызова
	-->
	...
	<!-- Подключение объекта запросов -->
	<script src="{{ asset('public/spa/scripts/query.js') }}"></script>
	...
	<!-- 
		...
		*	Функции:
			...
			* popup.message(message, color) - вызов небольшого окна с сообщение в правом верхнем углу сайта
...
<body>
	...
	<div id="mask"></div>
	<!-- Блок для вывода небольших сообщений -->
	<div id="message"></div>
